module.exports = {
    ...
    category: 'ai',
    description: 'Premium Kosem Bot AI Chat',
    usage: '.ai <question>',
    ...
    async execute(sock, msg, args, extra) {
        try {
            if (!args[0]) {
                ...
                errText += `💡 Please provide a prompt or question.\n`;
            ...
            for (let api of apis) {
                try {
                    ...
                    if (answer && answer.length > 2) {
                        finalAnswer = answer;
                        console.log(`[KOSEM AI] 🟢 Success!`);
                        break; // Stop looking, we got the answer
                    }
                } catch (err) {
                    console.log(`[KOSEM AI] 🔴 Failed: ${err.message}. Trying next...`);
                    continue; 
                }
            }
            ...
            if (finalAnswer) {
                ...
                if (extra.react) await extra.react('❌');
                await extra.reply("❌ *Host Restricted:* Your VPS IP is currently blacklisted and has been blocked by all available API providers. You may need to host your bot on a different panel.");
            }
